...
<video src="docs/game.mp4" controls width="720"></video>

[Watch gameplay (MP4)](docs/game.mp4)
...
| `src/lcd_timing.v` | 800x480 timing, border, `frame_start` |
...
| `src/fb_ram.v` | 2-bit 800x470 BRAM |
...
If you keep a separate Gowin tree, copy **all** of `fpga/tang20k_lcd/src/*.v` **and** `sw_config.vh` into that project. Keep `sw_config.vh` on the FileList (`` `define ``-only header). Add `sw_physics.v` / `sw_draw.v` / `sw_scanout.v` if missing, then rebuild. See `docs/Gowin-copy-bsram.txt`.
